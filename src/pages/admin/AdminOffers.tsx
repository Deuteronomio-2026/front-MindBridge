...
import { Plus, Tag, CheckCircle, XCircle, Calendar, User, Sparkles } from "lucide-react";
...
export type OfferStatus = "OPEN" | "TAKEN" | "CANCELLED";
...
const mockOffers: Offer[] = [
  {
    id: "1",
    title: "Oferta Abril",
    description: "El psicólogo suscrito aparecerá destacado en el listado de pacientes durante todo abril con mayor visibilidad.",
    benefits: ["Apareces primero en búsquedas", "Insignia 'Destacado' en tu perfil", "3× más impresiones", "Notificación a pacientes interesados"],
    boostMultiplier: 3,
    discountPercent: 20,
  ...
  },
  {
    id: "2",
    title: "Oferta Mayo",
    description: "Visibilidad premium durante mayo. Solo un psicólogo puede acceder a este beneficio exclusivo.",
    benefits: ["Posición destacada en listado", "Banner en página principal", "2× más impresiones"],
    boostMultiplier: 2,
    discountPercent: 15,
  ...
  },
  {
    ...
    benefits: ["Posición destacada", "2× impresiones"],
    boostMultiplier: 2,
    discountPercent: 20,
...
export default function AdminOffers() {
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    setTitle(""); setDescription(""); setBenefitsRaw("");
    setBoostMultiplier(2); setDiscountPercent(15);
      ...
      <div style={{ background: `linear-gradient(135deg, #0D2E38 0%, ${TEAL} 100%)` }} className="py-10 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex gap-4 mt-5">
            {[{ label: "Abiertas", count: openOffers.length }, { label: "Tomadas", count: takenOffers.length }].map((s) => (
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-4">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                placeholder="Ej: Oferta Abril 2026" required
            ...
            <div>
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                placeholder="Describe el beneficio que obtendrá el psicólogo suscrito..." required rows={3}
            ...
            <div>
              ...
              <input type="text" value={benefitsRaw} onChange={(e) => setBenefitsRaw(e.target.value)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  {[2, 3, 4, 5].map((n) => (
                    <button key={n} type="button" onClick={() => setBoostMultiplier(n)}
                      className="flex-1 py-2.5 rounded-xl border transition-colors"
                      style={{ background: boostMultiplier === n ? TEAL : "white", color: boostMultiplier === n ? "white" : "#4a6572", borderColor: boostMultiplier === n ? TEAL : "rgba(26,74,92,0.2)", fontSize: "0.82rem", fontWeight: 700 }}>
                      {n}×
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[10, 15, 20, 25].map((n) => (
                    <button key={n} type="button" onClick={() => setDiscountPercent(n)}
                      className="flex-1 py-2.5 rounded-xl border transition-colors"
                      style={{ background: discountPercent === n ? CORAL : "white", color: discountPercent === n ? "white" : "#4a6572", borderColor: discountPercent === n ? CORAL : "rgba(26,74,92,0.2)", fontSize: "0.82rem", fontWeight: 700 }}>
                      {n}%
                    </button>
                  ))}
                </div>
