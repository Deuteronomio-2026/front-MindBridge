...
import {
  ...
  Bell, ChevronDown, LogOut, Settings, Sparkles, Users
} from "lucide-react";
...
const navLinks = [
  { href: "/panel-psicologo", label: "Panel", icon: LayoutDashboard, exact: true, group: "main" },
  { href: "/panel-psicologo/citas", label: "Mis Citas", icon: Calendar, group: "main" },
  { href: "/panel-psicologo/agenda", label: "Agenda", icon: CalendarDays, group: "main" },
  { href: "/panel-psicologo/agenda/editar", label: "Editar Agenda", icon: Edit, group: "main" },
  { href: "/panel-psicologo/perfil", label: "Mi Perfil", icon: User, group: "profile" },
  { href: "/panel-psicologo/ofertas", label: "Ofertas", icon: Sparkles, group: "offers" },
  { href: "/panel-psicologo/sesiones-grupales", label: "Sesiones Grupales", icon: Users, group: "offers" },
];
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        console.error("Error fetching psychologist notifications:", error);
      } finally {
        setFetching(false);
      }
    };
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    <div key={link.href} className="flex items-center">
                      {showSeparator && <div className="w-px h-6 bg-gray-200 mx-2" />}
                      <Link
                        to={link.href}
                        className="px-5 py-2 rounded-lg transition-colors no-underline flex items-center gap-2"
                        style={{
